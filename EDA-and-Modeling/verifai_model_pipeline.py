...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        prog = 'VerifAI',
        description = 'Automatically verifai facts',
        epilog = 'Text at bottom of help'
    )

    parser.add_argument('--text', type=str)
    parser.add_argument('--oneclaim', type=bool)
    parser.add_argument('--searchversion', type=int)
    parser.add_argument('--llm', type=str)

    args = parser.parse_args()

    text_to_judge = args.text
    one_claim_arg = args.oneclaim
    search_version_arg = args.searchversion
    llm_to_use = args.llm
 

    # print(text_to_judge)
    # print(one_claim_arg)
    # print(search_version_arg)
    
    # Run the pipeline
    out = run_claim_judge_pipeline(
        text = text_to_judge, 
        search_version = search_version_arg,
        one_claim = one_claim_arg, 
        llm_to_use = llm_to_use,
    )

    import pprint
    pprint.pp(out['final_output'])
    # print('Response from the bot:')
    # for line in out:
    #     print('>', line[0])
